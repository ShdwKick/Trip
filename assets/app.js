...
function renderTrips() {
  ...
  trips.forEach((t, i) => {
    ...
    if (t.myRole === "owner") {
      ...
      // В подвал, а не поверх карточки: метка «моя» стоит там же у правого края
      // и появляется по тому же условию — наложенная кнопка перекрывала её.
      card.querySelector(".tc-foot").append(del);
    }
    grid.append(card);
  });
}
