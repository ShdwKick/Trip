...
7. guest — метка блока, если чек разбит по гостям («Гость 1», «Гость 2»). Иначе null.
8. Если какая-то цифра не читается — верни null в этом поле, не выдумывай. Название, которое не читается, тоже не додумывай: перепиши как видишь.`;
...
function splitPortions(items) {
  ...
  for (const item of items) {
    ...
    const each = Number.isFinite(qty) && qty >= 2 && qty <= 10 && Number.isInteger(qty)
